...
struct EllegnoteLiveActivity: Widget {
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: EllegnoteAttributes.self) { context in
            // Lock Screen UI / Banner pri nepodporovanom Dynamic Islande
            HStack(spacing: 12) {
            ...
            .activitySystemActionTint(Color.black)
            
        } dynamicIsland: { context in
            DynamicIsland {
                // 1. Expanded Režim (Rozbalený po podržaní prsta)
                ...
                DynamicIslandExpandedRegion(.bottom) {
                    HStack {
                        ...
                        Spacer()
                        // Časovač od poslednej aktualizácie
                ...
                // 2. Compact Leading (Ľavá strana zatvorenej pilulky)
                HStack(spacing: 4) {
                    Image(systemName: "figure.socialdance")
                ...
                // 3. Compact Trailing (Pravá strana zatvorenej pilulky)
                Text("\(context.state.currentFigureIndex)/\(context.state.totalFigures)")
                    .font(.system(size: 10, weight: .black))
                    .foregroundColor(Color(red: 1.0, green: 0.84, blue: 0.0))
            } minimal: {
                // 4. Minimal (Samostatný krúžok na boku pri viacerých spustených Live Activities)
                Image(systemName: "figure.socialdance")
                    .font(.system(size: 11, weight: .bold))
                    .foregroundColor(Color(red: 0.95, green: 0.26, blue: 0.21))
            }
