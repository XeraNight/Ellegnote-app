                ...
                ForEach(VaultFilter.allCases) { filter in
                    Button {
                        ...
                        Text(filter.rawValue)
                            ...
                            .padding(.horizontal, 12)
                            .padding(.vertical, 6)
                            .background(selectedFilter == filter ? Color.themeDark : Color.themeBgCard)
                            .foregroundColor(selectedFilter == filter ? .white : .themeDark)
                            .cornerRadius(20)
                            .overlay(
                                RoundedRectangle(cornerRadius: 20)
                                    .stroke(Color.themeDark.opacity(0.15), lineWidth: 1.5)
                            )
                    }
                }
            }
    ...
    private func videoCardView(for entry: VideoMediaEntry) -> some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(isSlotA ? Color.red : (isSlotB ? Color.green : Color.themeDark.opacity(0.12)), lineWidth: (isSlotA || isSlotB) ? 2 : 1)
        )
    }
    ...
    private var floatingActionBar: some View {
        HStack(spacing: 12) {
            ...
            PhotosPicker(selection: $selectedPhotoItem, matching: .videos) {
                ...
                .cornerRadius(12)
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(Color.themeDark.opacity(0.2), lineWidth: 1.5))
